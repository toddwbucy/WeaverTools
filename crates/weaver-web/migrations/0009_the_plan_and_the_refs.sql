...
--
-- **A run and an artifact do not take this**, and that is the convention
-- rather than an exception to it: section 2.2 has a run's identity as the
-- record spells it and section 2.3 computes an artifact's at import, so a
-- prefixed spelling would be an identity written over one received. **The
-- shape of a key says whether the row was authored here or landed here.**
CREATE TABLE plan (
  plan_id        TEXT PRIMARY KEY
                 DEFAULT ('pl-' || substr(replace(gen_random_uuid()::text, '-', ''), 1, 16)),
  parent_run_id  TEXT NOT NULL REFERENCES run(run_id),
  author         TEXT,
  version        BIGINT NOT NULL DEFAULT 1,

  CONSTRAINT plan_id_is_shaped CHECK (plan_id ~ '^pl-[0-9a-f]{16}$')
);
...
CREATE TABLE plan_arm (
  ...
  arm_id         TEXT PRIMARY KEY
                 DEFAULT ('ar-' || substr(replace(gen_random_uuid()::text, '-', ''), 1, 16)),
  ...
  CONSTRAINT arm_id_is_shaped CHECK (arm_id ~ '^ar-[0-9a-f]{16}$')
);
...
CREATE TABLE plan_entry (
  arm_id       TEXT NOT NULL REFERENCES plan_arm(arm_id) ON DELETE CASCADE,
...
CREATE TABLE ref (
  ref_id   TEXT PRIMARY KEY
           DEFAULT ('rf-' || substr(replace(gen_random_uuid()::text, '-', ''), 1, 16)),
  name     TEXT NOT NULL,
  ...
  CONSTRAINT ref_id_is_shaped CHECK (ref_id ~ '^rf-[0-9a-f]{16}$')
);
