...
evidence G6 rests on, and G6 is what makes the quarry's deletion safe. `weaver-spu-Spec`
cites it as its quarry map, which is provenance rather than a dependency: that Spec
elects against the merged corpus and would not change if this survey were wrong.
