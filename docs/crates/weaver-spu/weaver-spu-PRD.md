...
the mechanism the Spec's per family as the flush's is. The span is state
and never the record. This crate judges nothing, which is 13.9's rule that
cognition is the loop's facing a third way, and the identity prefix bounds
